...
const float lightIntensity = 60.0;

//LECTURE 10c - Toon shading
const int bands = 6;
const float scaleFactor = 2.0 / bands;
...
void main() {
	...
	if (u_Option1){
		...
		result = ((u_AmbientCol * u_AmbientStrength) + (ambient + diffuse) * attenuation) * inColor * textureColor.rgb; // Object color
	}if (u_Option3){
		result = ((specular + diffuse) *attenuation)*inColor*textureColor.rgb;
	}if (u_Option4){
		result = (
		...
		result = ((u_AmbientCol * u_AmbientStrength) + (ambient + specular) * attenuation) * diffuseOut * textureColor.rgb;
	}
