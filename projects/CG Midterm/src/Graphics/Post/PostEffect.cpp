...
void PostEffect::Init(unsigned width, unsigned height)
{
	...
	_shaders[_shaders.size() - 1]->LoadShaderPartFromFile("shaders/passthrough.vert", GL_VERTEX_SHADER);
	_shaders[_shaders.size() - 1]->LoadShaderPartFromFile("shaders/passthrough.frag", GL_FRAGMENT_SHADER);
