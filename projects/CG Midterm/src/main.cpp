...
#pragma region Shader and ImGui
		...
		glm::vec3 lightPos = glm::vec3(0.0f, -1.88f, 6.37f);
		glm::vec3 lightCol = glm::vec3(1.0f, 1.0f, 1.0f);
		float     lightAmbientPow = 0.8f;
		float     lightSpecularPow = 0.7f;
		glm::vec3 ambientCol = glm::vec3(1.0f);
		float     ambientPow = 0.52f;
		float     lightLinearFalloff = 0.5f;
		float     lightQuadraticFalloff = 0.6f;
		...
		glm::fvec2 BlurValues= glm::fvec2(0,0);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Midterm Bloom Lighting Settings"))
			{
				...
				if (ImGui::SliderFloat("Horizontal Blur Value", &BlurValues.x, 0.0f, 1.0f)) {}
				if (ImGui::SliderFloat("Vertical Blur Value", &BlurValues.y, 0.0f, 1.0f)) {}
			}
...
#pragma region Scene Generation
		...
		GameScene::sptr scene = GameScene::Create("ICG Midterm Jonathan Narine 100741302");
		...
		GameObject obj1 = scene->CreateEntity("Grass");
		{
			...
			obj1.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.5f);
		...
		GameObject obj4 = scene->CreateEntity("Trees");
		{
			...
			obj4.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.5f);
		...
		GameObject obj5 = scene->CreateEntity("Well");
		{
			...
			obj5.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.5f);
...
#pragma endregion 
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			if (glfwGetKey(BackendHandler::window, GLFW_KEY_6) == GLFW_PRESS)
			{
